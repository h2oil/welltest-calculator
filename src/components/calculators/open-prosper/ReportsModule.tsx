...
Max Rate: ${nodalResult.ipr_curve?.max_rate ? Number(nodalResult.ipr_curve.max_rate.toFixed(1)) : 'N/A'} ${getFlowRateUnit()}
Max Pressure: ${nodalResult.ipr_curve?.max_pressure ? Number(nodalResult.ipr_curve.max_pressure.toFixed(0)) : 'N/A'} ${getPressureUnit()}
Data Points: ${nodalResult.ipr_curve.rates.length}

VLP CURVE
=========
Correlation: ${case_.vlp.correlation}
Max Rate: ${nodalResult.vlp_curve?.max_rate ? Number(nodalResult.vlp_curve.max_rate.toFixed(1)) : 'N/A'} ${getFlowRateUnit()}
Max Pressure: ${nodalResult.vlp_curve?.max_pressure ? Number(nodalResult.vlp_curve.max_pressure.toFixed(0)) : 'N/A'} ${getPressureUnit()}
Data Points: ${nodalResult.vlp_curve.rates.length}

COMPLETION DETAILS
==================
Tubing ID: ${Number(case_.completion.tubing_id?.toFixed(3)) || 0} ${getLengthUnit()}
Tubing Roughness: ${Number(case_.completion.tubing_roughness?.toFixed(5)) || 0} ${getLengthUnit()}
Packer Depth: ${case_.completion.packer_depth ? Number(case_.completion.packer_depth.toFixed(0)) : 'Not set'} ${getLengthUnit()}
...
=============
  ...
  const generatePDF = async (content: string) => {
    ...
    addText(`Reservoir Pressure: ${Number(case_.ipr.parameters.reservoir_pressure?.toFixed(0)) || 0} ${unitSystem === 'metric' ? 'kPa' : 'psi'}`, 10);
    addText(`Reservoir Temperature: ${Number(case_.fluid.temperature?.toFixed(1)) || 0}°C`, 10);
    addText(`Tubing ID: ${Number(case_.completion.tubing_id?.toFixed(3)) || 0} ${unitSystem === 'metric' ? 'm' : 'in'}`, 10);
    addText(`Total Depth: ${Number(Math.max(...case_.deviation.map(d => d.md))?.toFixed(0)) || 0} ${unitSystem === 'metric' ? 'm' : 'ft'}`, 10);
    addText('', 10);

    // Operating Point
    addText('OPERATING POINT', 12, true);
    addText(`Flow Rate: ${nodalResult.operating_point?.rate ? Number(nodalResult.operating_point.rate.toFixed(1)) : 'N/A'} ${unitSystem === 'metric' ? 'm³/d' : 'bbl/d'}`, 10);
    addText(`Bottomhole Pressure: ${nodalResult.operating_point?.pwf ? Number(nodalResult.operating_point.pwf.toFixed(0)) : 'N/A'} ${unitSystem === 'metric' ? 'kPa' : 'psi'}`, 10);
    addText(`Wellhead Pressure: ${nodalResult.operating_point?.whp ? Number(nodalResult.operating_point.whp.toFixed(0)) : 'N/A'} ${unitSystem === 'metric' ? 'kPa' : 'psi'}`, 10);
    ...
    addText(`Max Rate: ${nodalResult.ipr_curve?.max_rate ? Number(nodalResult.ipr_curve.max_rate.toFixed(1)) : 'N/A'} ${unitSystem === 'metric' ? 'm³/d' : 'bbl/d'}`, 10);
    addText(`Max Pressure: ${nodalResult.ipr_curve?.max_pressure ? Number(nodalResult.ipr_curve.max_pressure.toFixed(0)) : 'N/A'} ${unitSystem === 'metric' ? 'kPa' : 'psi'}`, 10);
    ...
    addText(`Max Rate: ${nodalResult.vlp_curve?.max_rate ? Number(nodalResult.vlp_curve.max_rate.toFixed(1)) : 'N/A'} ${unitSystem === 'metric' ? 'm³/d' : 'bbl/d'}`, 10);
    addText(`Max Pressure: ${nodalResult.vlp_curve?.max_pressure ? Number(nodalResult.vlp_curve.max_pressure.toFixed(0)) : 'N/A'} ${unitSystem === 'metric' ? 'kPa' : 'psi'}`, 10);
    ...
    addText(`Tubing ID: ${Number(case_.completion.tubing_id?.toFixed(3)) || 0} ${unitSystem === 'metric' ? 'm' : 'in'}`, 10);
    addText(`Tubing Roughness: ${Number(case_.completion.tubing_roughness?.toFixed(5)) || 0} ${unitSystem === 'metric' ? 'm' : 'in'}`, 10);
    addText(`Packer Depth: ${case_.completion.packer_depth ? Number(case_.completion.packer_depth.toFixed(0)) : 'Not set'} ${unitSystem === 'metric' ? 'm' : 'ft'}`, 10);
    ...
    for (let i = 0; i < iprDataPoints; i++) {
      const rate = nodalResult.ipr_curve?.rates?.[i] ? Number(nodalResult.ipr_curve.rates[i].toFixed(1)) : 0;
      const pressure = nodalResult.ipr_curve?.pressures?.[i] ? Number(nodalResult.ipr_curve.pressures[i].toFixed(0)) : 0;
    ...
    for (let i = 0; i < vlpDataPoints; i++) {
      const rate = nodalResult.vlp_curve?.rates?.[i] ? Number(nodalResult.vlp_curve.rates[i].toFixed(1)) : 0;
      const pressure = nodalResult.vlp_curve?.pressures?.[i] ? Number(nodalResult.vlp_curve.pressures[i].toFixed(0)) : 0;
      addText(`${rate.padStart(8)} | ${pressure.padStart(8)}`, 9);
    }
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <div className="text-2xl font-bold">
                {nodalResult.operating_point?.rate ? Number(nodalResult.operating_point.rate.toFixed(1)) : 'N/A'}
              </div>
